...
def check_pg_package_schema_exists(cursor) -> None:
    cursor.execute(
        'SELECT EXISTS(\
...
def check_pg_package_tables_exist(cursor) -> None:
    tables: list[str] = [table.__name__ for table in mgr.tables]
    cursor.execute(
        'SELECT NOT EXISTS(\
...
def check_pg_package_functions_exist(cursor) -> None:
    functions: tuple[str, ...] = [function.__name__ for function in mgr.functions]
    cursor.execute(
        'SELECT NOT EXISTS(\
            SELECT req FROM unnest(%s::text[]) req\
...
def check_pg_package_installed_correctly(config: ActionConfiguration) -> None:
    with psycopg.connect(config.DB_DSN) as conn:
        with conn.cursor() as cursor:
            check_pg_package_schema_exists(cursor)
            check_pg_package_tables_exist(cursor)
            check_pg_package_functions_exist(cursor)
