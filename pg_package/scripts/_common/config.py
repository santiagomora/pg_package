...
import toml
import pg_package
import pg_definition
# from pg_package.backend import\
#     register_types
import functools
import time
...
class _ExecutionConfiguration:
    def __init__(
        ...
        sys.path.append('/home/smora/sgs/dev/pgdriver/test/')
        self.args = args
    ...
    def DATAFIX_PATH(self) -> str:
        return f'{os.path.dirname(self.package.__file__)}/{self.config["migration"]["datafix_relative_path"]}'
    ...
    def TEMPLATE_PATH(self) -> str:
        return f'{os.path.dirname(pg_package.__file__)}/template'
    ...
    #     with pg_definition.adapter_registry(self.DB_DSN) as ar:
    #         register_types(ar)
...
class ActionConfiguration(_ExecutionConfiguration):
    @abstractmethod
    def opposite_action(self) -> mgr.execution_action:
        pass
...
class DowngradeConfiguration(ActionConfiguration):
    def __init__(
        ...
        self.execution_action = mgr.execution_action.downgrade

    def opposite_action(self) -> mgr.execution_action:
        return mgr.execution_action.downgrade

    @staticmethod
    def compare_migrations(mgr1: 'MigrationWrapper', mgr2: 'MigrationWrapper') -> bool:
        return mgr2.NAME in mgr1.DEPENDS_ON
...
class UpgradeConfiguration(ActionConfiguration):
    def __init__(
        ...
        self.execution_action = mgr.execution_action.upgrade
    ...
    @staticmethod
    def compare_migrations(mgr1: 'MigrationWrapper', mgr2: 'MigrationWrapper') -> bool:
        return mgr2.NAME not in mgr1.DEPENDS_ON
...
class SetupConfiguration(ActionConfiguration):
    def __init__(
        ...
        ActionConfiguration.__init__(self, "pg_package", args)
        self.execution_action = mgr.execution_action.setup
