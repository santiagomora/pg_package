...
def create_migration_from_template(
    *, config: argparse.Namespace, dest_path: str, template_path: str,
    ...
    with open(PATH, 'w') as mgrdest:
        ...
        mgrdest.write(mgr_template.format(
...
if __name__ == '__main__':
    ...
    mgr_group_exc.add_argument('--with-datafix', help='Generate migration, datafix and bind with them together')
    mgr_group.add_argument('--depends-on', type=str, help='Indicate a comma separated list of existing migration names that this migration depends on')
    ...
    mgr_name: Optional[str] = create_migration_from_template(
        config=args, dest_path=CONFIG.MIGRATION_PATH,
        template_path=CONFIG.TEMPLATE_PATH,
        datafix_name=dfx_name
    )
    if dfx_name is None and mgr_name is None:
        print('ERROR')
        exit(1)
