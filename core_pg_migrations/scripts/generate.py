...
if __name__ == '__main__':
    ...
    core_pg_migrations_group = parser.add_argument_group('Migration Options')
    core_pg_migrations_group_exc = core_pg_migrations_group.add_mutually_exclusive_group()
    core_pg_migrations_group_exc.add_argument('--with-datafix', help='Generate migration, datafix and bind with them together')
    core_pg_migrations_group.add_argument('--depends-on', type=str, help='Indicate a comma separated list of existing migration names that this migration depends on')
    ...
    if dfx_name is None and core_pg_migrations_name is None:
        prompt_error('ERROR')
        exit(1)
