...
def create_migration_from_template(
    *, config: argparse.Namespace, dest_path: str, template_path: str,
    ...
    if os.path.exists(PATH):
        print(f'There is already a migration with name "{NAME}" at destination path "{dest_path}"')
        return False

    mgr_manager_template: str = ''
    with open(f'{template_path}/migration.tpl', 'r') as mgrtpl:
        mgr_manager_template = mgrtpl.read()
    ...
    for name in DEPENDS:
        ...
        if not os.path.exists(f'{dest_path}/{depends_filename}'):
            print(f'Unmet dependency. "{depends_filename}" must be a migration in path "{dest_path}"')
            errors = True
    ...
    with open(PATH, 'w') as mgrdest:
        ...
        mgrdest.write(mgr_manager_template.format(
...
def create_datafix_from_template(
    *, config: argparse.Namespace, dest_path: str, template_path: str
    ...
    if os.path.exists(PATH):
        print(f'There is already a datafix with name "{NAME}" at destination path "{dest_path}"')
        return None
...
if __name__ == '__main__':
    ...
    mgr_manager_group = parser.add_argument_group('Migration Options')
    mgr_manager_group_exc = mgr_manager_group.add_mutually_exclusive_group()
    mgr_manager_group_exc.add_argument('--with-datafix', help='Generate migration, datafix and bind with them together')
    mgr_manager_group.add_argument('--depends-on', type=str, help='Indicate a comma separated list of existing migration names that this migration depends on')

    df_group = parser.add_argument_group('Datafix Options')

    args = parser.parse_args(sys.argv[1:])

    CONFIG = GenerateConfiguration(args.package)
    ...
    mgr_manager_name: Optional[str] = create_migration_from_template(
        config=args, dest_path=CONFIG.MIGRATION_PATH,
        template_path=CONFIG.TEMPLATE_PATH,
        datafix_name=dfx_name
    )
    if dfx_name is None and mgr_manager_name is None:
        print('ERROR')
        exit(1)
