...
from .check_install import\
    check_mgr_manager_installed_correctly
...
class ExecutionHeap(list[MigrationWrapper]):
    ...
    def unfold_dependencies(exec_heap: list[MigrationWrapper]) -> list[MigrationWrapper]:
        ...
        def determine_dependencies(relations_matrix: list[list[int]], row: int) -> set[str]:
            ...
            for ix in range(0, len(relations_matrix[row])):
                if relations_matrix[row][ix]:
                    res = res.union(determine_dependencies(relations_matrix, ix))
            return res

        for ix in range(0, len(exec_heap)):
            setattr(exec_heap[ix], '_DEPENDS_ON', determine_dependencies(relations_matrix, ix))
...
def get_migration_execution_heap(
    ...
    check_mgr_manager_installed_correctly(config)
    if name is None:
